...
import '../component/presentationCard/PresentationCard';
import '../component/ContainerItem';
import { PageKeys } from '../config/pageKeys';
export default  class Native extends ProtectedRoute {
    ...
    render() {
        return html`
            ...
            <container-item>
                <presentation-card name=${this.user.name} photo=${this.user.photo} skills=${this.user.skills} hobbies=${this.user.hobbies} stack=${this.user.stack} age=${this.user.age} occupation=${this.user.occupation}></presentation-card-lit>
            </container-item>
        ` 
    }
}

customElements.define(PageKeys.NATIVE, Native);
